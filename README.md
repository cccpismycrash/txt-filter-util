...
прочих символов. *В том числе считается, что пустая строка, тоже является строкой*. То есть, если строка не попадает под `целочисленный` или `вещественный` тип, то она будет определена как `строковый` тип.
